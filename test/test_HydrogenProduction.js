...
describe("H2 Contract Testing", function () {
    ...
    describe('Transferring Energy', () => {
        ...
        it('Electrolysis transfers to distributor1 full batch', async function () {
            await expect(h2p.connect(electrolysis).transfer(distribution1.address,batchOfEnergy)).not.reverted
        });
        ...
        it('Batch of energy in circulation: 10000 ', async function () {
            expect (await h2p.totalSupply()).to.equal(batchOfEnergy * 2);
        });

        it('New batch is distributed to distributor 2 via electrolysis', async function () {
            await expect(h2p.transfer(electrolysis.address,batchOfEnergy)).not.reverted
            await expect(h2p.connect(electrolysis).transfer(distribution2.address,batchOfEnergy)).not.reverted
        });

        it(`Fast-forward: Both distributors forward it to despensing stations (1 and 2)`,async function () {
            await expect(h2p.connect(distribution1).transfer(dispensing1.address,batchOfEnergy)).not.reverted
            await expect(h2p.connect(distribution2).transfer(dispensing2.address,batchOfEnergy)).not.reverted
        });

        it(`Burning one batch of tokens`,async function () {
            await expect(h2p.connect(dispensing1).burn(batchOfEnergy)).not.reverted
        });
        ...
        it(`Burning last batch of tokens, and total supply in circulation should be zero `,async function () {
            await expect(h2p.connect(dispensing2).burn(batchOfEnergy)).not.reverted
            expect(await h2p.totalSupply()).to.equal(0)
        });
    });
});